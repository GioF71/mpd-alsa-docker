...
The `version-0.24.10` tag is in-line with the GitHub [upstream repo](https://github.com/MusicPlayerDaemon/MPD) at version 0.24.10.  
The `version-0.24.10-ups` tag contains a patch which is used when `INTEGER_UPSAMPLING` is set to `yes`. Use at your own risk.  
...
The current mpd version is `v0.24.10` when using [giof71/mpd-compiler-docker](https://github.com/GioF71/mpd-compiler-docker) as the base image ([Docker repo here](https://hub.docker.com/r/giof71/mpd-compiler)). The repo binary is installed also in this case.  
...
:---|:---|:---|:---|:---
giof71/mpd-compiler:bookworm|**latest**, **stable**, bookworm|0.24.10|[0.23.12](https://packages.debian.org/bookworm/mpd)|[0.24](https://packages.debian.org/bookworm/mpdscribble)
