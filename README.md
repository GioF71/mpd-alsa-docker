...
OUTPUT_MODE|Output mode, can be `alsa` (default), `pulse` (**deprecated**, you should use `PULSE_AUDIO_OUTPUT_CREATE` set to `yes`), `null` or `none`. For `pulse` mode, running in `user` mode is required. Note that `none` does not create any output, leaving the task to the variables dedicated to additional outputs
